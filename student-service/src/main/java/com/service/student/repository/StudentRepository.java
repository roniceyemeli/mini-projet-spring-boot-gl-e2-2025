...
public interface StudentRepository extends JpaRepository<Student, Long> {
    ...
    List<Student> findByCommunityId(Long communityId);
    ...
    @Query("SELECT s.schoolId, COUNT(s) FROM Student s WHERE s.enrollmentStatus = 'ACTIVE' GROUP BY s.schoolId")
    List<Object[]> countStudentsBySchool();

    @Query("SELECT s.program, COUNT(s) FROM Student s WHERE s.enrollmentStatus = 'ACTIVE' GROUP BY s.program")
    List<Object[]> countStudentsByProgram();

    @Query("SELECT s.enrollmentStatus, COUNT(s) FROM Student s GROUP BY s.enrollmentStatus")
    List<Object[]> countStudentsByEnrollmentStatus();

    @Query("SELECT s.academicLevel, COUNT(s) FROM Student s WHERE s.enrollmentStatus = 'ACTIVE' GROUP BY s.academicLevel")
    List<Object[]> countStudentsByAcademicLevel();

    @Query("SELECT AVG(s.gpa) FROM Student s WHERE s.gpa IS NOT NULL AND s.enrollmentStatus = 'ACTIVE'")
    Double averageGPA();

    @Query("SELECT SUM(s.completedCredits) FROM Student s WHERE s.enrollmentStatus = 'ACTIVE'")
    Integer sumCompletedCredits();

    @Query("SELECT s FROM Student s WHERE s.enrollmentStatus = 'ACTIVE' AND " +
            "s.expectedGraduationYear <= YEAR(CURRENT_DATE) AND " +
            "s.completedCredits >= s.totalCredits AND " +
            "s.isGraduated = false")
    List<Student> findEligibleForGraduation();

    @Query("SELECT s FROM Student s WHERE s.updatedAt < :cutoffDate AND s.isActive = true")
